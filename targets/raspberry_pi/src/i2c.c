...
static struct __i2c_status_s {
  picowjs_i2c_mode_t mode;
} __i2c_status[I2C_NUM];

static bool __check_i2c_pins(uint8_t bus, picowjs_i2c_pins_t pins) {
...
void picowjs_i2c_init() {
  for (int i = 0; i < I2C_NUM; i++) {
    __i2c_status[i].mode = PICOWJS_I2C_NONE;
  }
}
...
int picowjs_i2c_setup_master(uint8_t bus, uint32_t speed, picowjs_i2c_pins_t pins) {
  ...
  if ((i2c == NULL) || (__i2c_status[bus].mode != PICOWJS_I2C_NONE) ||
      (__check_i2c_pins(bus, pins) == false)) {
    return EDEVINIT;
  }
  __i2c_status[bus].mode = PICOWJS_I2C_MASTER;
...
int picowjs_i2c_setup_slave(uint8_t bus, uint8_t address, picowjs_i2c_pins_t pins) {
  ...
  __i2c_status[bus].mode = PICOWJS_I2C_SLAVE;
  return 0;
}
...
int picowjs_i2c_mem_write_master(uint8_t bus, uint8_t address, uint16_t mem_addr,
  ...
  int ret;
  uint8_t __memaddr[2];
  if ((i2c == NULL) || (__i2c_status[bus].mode != PICOWJS_I2C_MASTER)) {
    return EDEVWRITE;
  }
...
int picowjs_i2c_write_slave(uint8_t bus, uint8_t *buf, size_t len,
                       uint32_t timeout) {
  i2c_inst_t *i2c = __get_i2c_no(bus);
  if ((i2c == NULL) || (__i2c_status[bus].mode != PICOWJS_I2C_SLAVE)) {
    return EDEVWRITE;
  }
  return 0;
}
...
int picowjs_i2c_close(uint8_t bus) {
  ...
  i2c_deinit(i2c);
  __i2c_status[bus].mode = PICOWJS_I2C_NONE;
  return 0;
}
