...
static bool __check_uart_pins(uint8_t port, picowjs_uart_pins_t pins) {
  if (port == 0) {
...
int picowjs_uart_setup(uint8_t port, uint32_t baudrate, uint8_t bits,
  ...
  if ((flow & PICOWJS_UART_FLOW_RTS) && (pins.rts >= 0)) {
    rts_en = true;
    gpio_set_function(pins.rts, GPIO_FUNC_UART);
  }
  if ((flow & PICOWJS_UART_FLOW_CTS) && (pins.cts >= 0)) {
    cts_en = true;
    gpio_set_function(pins.cts, GPIO_FUNC_UART);
  }
  uart_set_hw_flow(uart, cts_en, rts_en);
  if (parity == PICOWJS_UART_PARITY_TYPE_EVEN) {
    pt = UART_PARITY_EVEN;
  } else if (parity == PICOWJS_UART_PARITY_TYPE_ODD) {
    pt = UART_PARITY_ODD;
  }
